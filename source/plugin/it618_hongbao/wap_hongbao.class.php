...
class mobileplugin_it618_hongbao {
	...
	function forumdisplay_thread_mobile_output(){
		...
		foreach($threadlist as $id => $thread){
			if(($it618_hongbao_main=DB::fetch_first("SELECT * FROM ".DB::table('it618_hongbao_main')." WHERE it618_tid=".$thread[tid]))){
				...
				$hongbao_listtips='<div style="font-size:13px;padding-left:10px;color:#999;padding-top:10px">'.$it618_hongbao_lang['s60'].'</div>';
				...
				if($postlist[$id]['first']==1){
					...
					if(in_array($groupid, $hongbao_groups)){
						...
						if($flag==0&&$postlist[$id]['authorid']==$_G['uid']){
							$tmparr[]=$tmpjs.'<table width="100%"><tr><td style="text-align:right;font-size:13px;margin-right:6px;cursor:pointer;margin-top:-1px" onclick="it618_showhongbao(\'it618_showhongbao\',\''.$_G['siteurl'].'plugin.php?id=it618_hongbao:showhongbao&it618_tid='.$_G['tid'].'&fid='.$_G['fid'].'&type='.$tmpcount.'\');"><img src="source/plugin/it618_hongbao/images/logo.png" height="18" style="vertical-align:middle;margin-top:-3px;margin-right:3px">'.$tmptitle.'</td></tr></table>';
						}
					...
					$tmpmobiletpl=$_G['mobiletpl'][IN_MOBILE];
					$_G['mobiletpl'][IN_MOBILE]='/';
					include template('it618_hongbao:popup');
					if($tmpcount>0) include template('it618_hongbao:hongbao');
					$_G['mobiletpl'][IN_MOBILE]=$tmpmobiletpl;
				
					
					
					if($it618_hongbao['hongbao_wapstyle']==1){
						$post['message']=$it618_hongbao_block.$popup_block.$post['message'];
					}else{
						$post['message']=$post['message'].'<br>'.$it618_hongbao_block.$popup_block;
					}
