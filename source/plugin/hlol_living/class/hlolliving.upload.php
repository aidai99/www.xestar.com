 ...
 *
 * $Id: hlolliving.upload.php 2018-1-2 10:45:11Z 风清扬 $
 */
...
Class hlolliving_upload{
	...
	function init($attach, $type = 'hlolliving', $extid = 0, $forcename = '') {
	...
	function get_image_info($target, $allowswf = false) {
		$ext = hlolliving_upload::fileext($target);
		$isimage = hlolliving_upload::is_image_ext($ext);
	...
	function get_target_dir($type, $extid = '', $check_exists = true) {
		...
		if($type == 'album' || $type == 'forum' || $type == 'portal' || $type == 'category' || $type == 'profile' || $type == 'hlolliving') {
			$subdir1 = date('Ym');
			$subdir2 = date('d');
			$subdir = $subdir1.'/'.$subdir2.'/';
		...
		$check_exists && hlolliving_upload::check_dir_exists($type, $subdir1, $subdir2);

		return $subdir;
	}

	function check_dir_type($type) {
		return !in_array($type, array('forum', 'group', 'album', 'portal', 'common', 'temp', 'category', 'profile','hlolliving')) ? 'temp' : $type;
	}

	function check_dir_exists($type = '', $sub1 = '', $sub2 = '') {

		$type = hlolliving_upload::check_dir_type($type);
		...
		if(!$res) {
			$res = $typedir && hlolliving_upload::make_dir($typedir);
			$res && $subdir1 && ($res = hlolliving_upload::make_dir($subdir1));
			$res && $subdir1 && $subdir2 && ($res = hlolliving_upload::make_dir($subdir2));
		}
	...
	function save_to_local($source, $target) {
		if(!hlolliving_upload::is_upload_file($source)) {
			$succeed = false;
