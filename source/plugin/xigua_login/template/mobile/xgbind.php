...
<form id="wechatform" class="form-control" method="post" autocomplete="off" action="plugin.php?id=xigua_login:xgbind&rfr={eval echo urlencode(dreferer());}">
    ...
    <div class="password login_from">
        <ul>
            <li>
                <label>{lang wechat:wechat_spacecp_pw}</label>
                <input type="password" name="newpassword1" size="30" class="px p_fre" tabindex="1" />

            </li>
            <li>
                <label>{lang wechat:wechat_spacecp_repw}</label>
...
<div id="unbind">
    <form id="wechatform" class="form-control" method="post" autocomplete="off" action="plugin.php?id=xigua_login:xgbind&rfr={eval echo urlencode(dreferer());}">
        ...
        <p class="btn_login">
        <button type="submit" name="unbindsubmit" value="yes" class="btn btn-outline"><strong>{lang wechat:wechat_spacecp_unbind_button}</strong></button>
        </p>
        <!--{/if}-->
    </form>
</div>
