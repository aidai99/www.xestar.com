...
<head>
    ...
    <style>
        ...
        .buttons-tab{position:relative;background:#fff;display:-webkit-box;display:-webkit-flex;display:-ms-flexbox;display:flex;width:100%;background-color:rgba(247,247,248,.95);height:50px;overflow:hidden;line-height:50px}
        .buttons-tab:before{content:" ";position:absolute;left:0;bottom:0;width:100%;height:1px;border-top:1px solid #979797;color:#979797;-webkit-transform-origin:0 0;transform-origin:0 0;-webkit-transform:scaleY(.5);transform:scaleY(.5)}
        ...
        .buttons-tab .a:after{content: " ";position: absolute;left: 0;top: 0;width: 1px;height:49px;border-left: 1px solid #979797;color: #979797; -webkit-transform-origin: 0 0; transform-origin: 0 0;-webkit-transform: scaleX(.5);transform: scaleX(.5);
        }
...
<div class="container_map container_map_ami">
    ...
    <div class="buttons-tab">
        <a class="a <!--{if $_GET[rb]==0}-->active<!--{/if}-->" href="plugin.php?id=xigua_login:reg&rb=0">$navtitle1</a>
        <a class="a <!--{if $_GET[rb]==1}-->active<!--{/if}-->" href="plugin.php?id=xigua_login:reg&rb=1">$navtitle2</a>
    </div>
    <!--{if $_GET[has]}-->
    ...
    <form  class="form-control" action="plugin.php?id=xigua_login:reg" method="post" <!--{if $config[charset]}-->accept-charset="{$config[charset]}"<!--{/if}-->>
        ...
        <div class="login_from">
            <ul>
                <li>
                    <input type="text" value="<!--{if $_GET[rb]==0}-->{$reg_nickname}<!--{/if}-->" tabindex="1" class="px p_fre" size="30" autocomplete="off" name="username" placeholder="{lang inputyourname}" >
                    {if $config[showborder]}<div class="border"></div>{/if}
                </li>

                <!--{if $_GET[rb]!=0}-->
                <li class="btn_login"><input type="password" tabindex="2" class="px p_fre" size="30" value="" name="password" placeholder="{lang login_password}" >
                    {if $config[showborder]}<div class="border"></div>{/if}
                </li>
                <li class="questionli">
