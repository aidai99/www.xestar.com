...
if(str_replace('X', '', DISCUZ_VERSION) >= 3.1){
		$_GET['step'] = 'sql';
		$_GET['deletesql'] = '201311162014I1LiEIZ5';
}
if($_GET['available']){
...
switch($_GET['step']){
	default:
	case 'check':
		$available = dfsockopen('http://www.zheyitianshi.com/api/available.php?siteurl='.rawurlencode($_G['siteurl']));
	...
	case 'sql':
		...
		$sql = <<<EOF
DROP TABLE IF EXISTS cdb_study_sharetoweixin_count;
DROP TABLE IF EXISTS cdb_study_sharetoweixin_view;
EOF;
		runquery($sql);
	...
	case 'addon':
		if($_GET['available']){
				...
				$_statInfo = base64_encode(serialize($_statInfo));
				$_md5Check = md5($_statInfo);
				$StatUrl = 'http://www.zheyitianshi.com/api/outer_addon.php';
